...
"제안서(임시)" 탭은 report/proposal.py 가 제안카드.md 를 읽어 조립한 절을
같은 방식(좌측 목차·우측 본문·auto/human 배지)으로 보여준다. **카드 파일은
여기서 읽기만 한다** — 사람이 쓰는 절의 저장은 st.session_state 에만 남고
제안카드.md 는 고치지 않는다.
"""
...
from report import proposal as Pr, sections as S, to_pdf
...
with tab_report:
    ...
    with body:
        ...
        if sec["kind"] == "todo":
            ...
            body_text = S.render_limits(t, st.session_state.limits_rows)
            bad = S.check_phrasing(body_text)
            if bad:
...
with tab_proposal:
    ...
    st.caption("제안카드.md 를 읽어 조립합니다. 이 화면에서는 카드 파일을 "
              "고치지 않습니다 — 사람이 쓴 절은 이 세션에만 저장됩니다.")

    cards = Pr.load_cards()
    if not cards.get("items"):
        ui.callout("제안카드.md 를 못 찾았거나 카드가 비어 있습니다 — "
                  "/제안 을 먼저 돌리십시오.")
        st.stop()

    psecs = Pr.build(cards, st.session_state.proposal_human)

    pnav, pbody = st.columns([1, 3.4])

    with pnav:
        ptitles = [s["title"] for s in psecs]
        ppick = st.radio("제안서 목차", ptitles, label_visibility="collapsed",
                         key="proposal_nav")
        st.divider()
    ...
    with pbody:
        ...
        plvl = "ok" if psec["kind"] == "auto" else (
            "ok" if psec["body"].strip() else "warn")
        st.markdown(
            f'<div style="display:flex;align-items:center;gap:12px;margin-bottom:10px">'
        ...
        if psec["kind"] == "auto":
            ...
            for tbl in psec.get("tables", []):
                if tbl.get("caption"):
                    st.caption(tbl["caption"])
                st.dataframe(pd.DataFrame(tbl["rows"]), width="stretch",
                            hide_index=True)
        else:
            if psec.get("guide"):
                ui.callout("<b>참고 가이드</b><br>" +
                          "<br>".join(f"· {line}" for line in psec["guide"]), "info")
            draft = st.text_area(
                psec["title"], value=psec["body"], height=200,
                placeholder=psec.get("placeholder", ""),
                key=f"proposal_form_{psec['title']}", label_visibility="collapsed")
            if st.button("저장", key=f"proposal_save_{psec['title']}", type="primary"):
                st.session_state.proposal_human[psec["title"]] = draft
                flagged = S.check_phrasing(draft)
                if flagged:
                    ui.callout("저장했습니다. 다만 인과를 단정하는 표현이 있습니다 — "
                              f"{', '.join(flagged)}. 관측 데이터로는 인과를 "
                              "주장할 수 없습니다.")
                else:
                    st.success("저장했습니다. 검사 통과.")
    ...
    st.divider()
    ui.section("내보내기")
    st.caption("지금까지 저장한 사람 작성 절(§5·§6)을 그대로 포함해 내보냅니다. "
              "카드 파일(제안카드.md)은 여기서도 고치지 않습니다 — 읽어서 조립만 합니다.")

    pc1, pc2 = st.columns(2)
    with pc1:
        st.markdown("**HTML** — 제안서_템플릿.html 구조 그대로")
        st.download_button(
            "제안서.html 다운로드", Pr.to_html(psecs),
            file_name="제안서.html", mime="text/html")

    with pc2:
        st.markdown("**PDF** — 표지 · 목차 · 카드 블록")
        if st.button("PDF 만들기", type="primary", key="proposal_pdf_build"):
            pdf = None
            with st.status("제안서를 만드는 중", expanded=True) as box:
                try:
                    st.write("1) 절 내용 모으는 중")
                    # psecs 는 위에서 이미 조립됨 — 사람 작성 절(§5·§6) 반영된 상태다.
            ...
            if pdf is not None:
                ...
                # ⚠ 파일 이름에만 쓰는 화면 표시용 시각이다 — 지표·판정 계산에는
                # 현재 시각을 넣지 않는다(리포트 탭과 같은 원칙).
                st.session_state.proposal_pdf_made_at = datetime.now(C.KST)
                st.toast("제안서 PDF가 만들어졌습니다", icon="📄")
        if st.session_state.get("proposal_pdf"):
            made_at = st.session_state.proposal_pdf_made_at
            st.download_button(
                "제안서.pdf 다운로드", st.session_state.proposal_pdf,
                file_name=f"제안서_{made_at:%Y%m%d-%H%M%S}.pdf",
                mime="application/pdf")
